 ...
 *  transceiver control callbacks to handle RS-485 transceiver control.
 *
...
{
	rs485->flags = SER_RS485_RX_DURING_TX | SER_RS485_RTS_ON_SEND;
}
...
{
	int line;
	dev_dbg(port->dev, ">ni16550_register_port\n");

	port->txvr_ops = &ni16550_txvr_ops;
	ni16550_set_rs485_defaults(&port->rs485);

	line = serial8250_register_port(port);
	dev_vdbg(port->dev, "line: 0x%08x\n", line);
	if (line < 0) {
		dev_dbg(port->dev, "line is less than zero\n");
		dev_dbg(port->dev, "<ni16550_register_port\n");
		return -ENODEV;
	}
