...
{
	return ioread32(drvdata.devcfg_addr + DEVCFG_INT_STS_OFFSET) & INT_STS_FPGA_DONE_MASK;
}
 ...
 * Returns 0 on success, negative error otherwise
 */
static int __devinit fpgaperipheral_probe(struct platform_device *pdev)
{
	int retval;
	void __iomem *devcfg_addr;
 ...
 * Returns 0 on success, negative error otherwise
 */
static int __devexit fpgaperipheral_remove(struct platform_device *pdev)
{
	struct resource *devcfg_res;
...
#ifdef CONFIG_OF
static struct of_device_id fpgaperipheral_of_match[] __devinitdata = {
